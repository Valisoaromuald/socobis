...
public class Prevision extends MvtCaisse {
    ...
    public String toString() {
        return "Prevision [id=" + id + ", compte=" + compte + ", soldeInitial=" + soldeInitial + ", soldeFinale="
                + soldeFinale + ", idFacture=" + idFacture + ", idDevise=" + idDevise 
                + ", getDebit()=" + getDebit() + ", getCredit()=" + getCredit() + "]";
    }
    ...
    public String getDatyGsemaine() {
        ...
        // Set the calendar to the start of the week (optional: change to Monday or
        // Sunday based on locale)
    ...
    public FactureCF getFactureClientouFournisseur(String ntable, Connection c) throws Exception {
        if (this.getIdFacture() == null)
            return null;
        if (this.isRecette()) {
            if (ntable == null || ntable.compareToIgnoreCase("") == 0)
                ntable = "vente_cpl";
        ...
        if (this.isDepense()) {
            if (ntable == null || ntable.compareToIgnoreCase("") == 0)
                ntable = "FACTUREFOURNISSEUR_MERECMPT";
    ...
    public Prevision decaler(double montantDebit, double montantCredit, String devise, Date datyVaovao, String u,
    ...
    public Prevision[] scinder(int nombre, String u, Connection c) throws Exception {
        ...
        for (int i = 0; i <= nombre - 2; i++) {
            ...
            duplication.setDaty(utilitaire.Utilitaire.ajoutJourDate(this.getDaty(),
                    utils.ConstanteStation.nombreJourScindeDefaut * (i + 1)));
    ...
    public static Prevision genererPrevision(ClassMAPTable o) throws Exception {
        if (o.getNomChampTotalPrev() == null)
            throw new Exception("Nom de champ de total de prevision non defini");
    ...
    public void controlerUpdate(Connection c) throws Exception {
        MvtCaissePrevision[] mvt = (MvtCaissePrevision[]) CGenUtil.rechercher(new MvtCaissePrevision(), null, null, c,
                " and id2='" + this.getId() + "'");
    ...
    public void scinder(String[] ids, String u, Connection c) throws Exception {
        ...
        try {
            ...
            if (ids == null || ids.length == 0)
                throw new Exception("veuiller cocher");
            for (int i = 0; i < ids.length; i++) {
                ids[i] = Utilitaire.split(ids[i], "_")[0];
            }
            Prevision[] lp = (Prevision[]) CGenUtil.rechercher(new Prevision(), null, null, c,
                    utilitaire.Utilitaire.getAWhereIn(ids, "id"));
            for (Prevision p : lp) {
                p.scinder(u, c);
            }
            if (estOuvert)
    ...
    public void modifierPP(String refuser, String[] id, String[] debits, String[] credits, String[] taux, String[] daty,
            Connection c) throws Exception {
        int indice = 0;
        try {
            if (c == null) {
            ...
            Prevision[] cmds = (Prevision[]) CGenUtil.rechercher(new Prevision(), null, null, c,
                    " and ID in (" + tid + ") order by id asc");
            if (cmds.length == 0) {
